...
class MobileNet(BaseMobileNet):
    def make_layers(self):
        """

        """
        yield Conv2D(3, (3, 3), padding="valid", use_bias=False, strides=(2, 2), name="conv2d_0")
        yield self.make_depthwise(filters=6)
        yield self.make_depthwise(filters=12, stride=2)
        yield self.make_depthwise(filters=12)
        yield self.make_depthwise(filters=24, stride=2)
        yield self.make_depthwise(filters=24)
        yield self.make_depthwise(filters=48, stride=2)
        yield self.make_depthwise(filters=48)
        yield self.make_depthwise(filters=48)
        yield self.make_depthwise(filters=48)
        yield self.make_depthwise(filters=48)
        yield self.make_depthwise(filters=96, stride=2)
        yield self.make_depthwise(filters=96)
